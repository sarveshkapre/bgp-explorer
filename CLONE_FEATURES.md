...
- 2026-02-11: Cleanup/refactor pass: de-duplicate `/api/bgp/lookup` source-evidence mapping and shared fetch options, centralize positive env-int parsing for rate-limit/cache settings, remove repeated prefix route href calculations in UI, and add a contract test for invalid env fallback defaults. Evidence: `src/app/api/bgp/lookup/route.ts`, `src/app/api/bgp/lookup/route.test.ts`, `src/app/bgp/ui.tsx`, `npm run lint && npm run typecheck && npm test && npm run build` (30 tests passed). (commit: `7e801e6`)
